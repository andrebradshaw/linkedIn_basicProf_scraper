...
function checkElmText(elm,n){if(elm[n] != undefined && elm[n] != null){return elm[n].innerText.trim();}else{ return '';}}
...
function fullObject(){
setTimeout(()=>{
var linkedinProfile = {"pidInformation": pidInformation, "experience": experience, "education": education, "skills": skills, "recommendations": recommendations, "accomplishments": accomplishments};

console.log(JSON.stringify(linkedinProfile));
},3900);
}
var rxs = /^.{0,9}(?=\s+–)/;
var rxe = /(?<=\s+–\s+).{0,9}/;
  ...
  for (w = 0; w < obj.length; w++) {
	...
	let start = dateParser(vld(rxs.exec(dates),0));
		let end = dateParser(vld(rxe.exec(dates),0));
          experience.push({"companyName": companyName, "companyId": companyId, "jobTitle": jobTitle, "geo": geo, "start": start, "end":end, "description": desc});
  }
}
...
function parseEdu(obj) {
  for (w = 0; w < edu_exp.length; w++) {
	...
	let dates = checker2(edu_exp[w].getElementsByClassName("pv-entity__dates"),0);
		let start = dateParser(vld(rxs.exec(dates),0));
		let end = dateParser(vld(rxe.exec(dates),0));
	education.push({"eduName": eduName, "eduId": eduId, "eduDeg": eduDeg, "start": start, "end": end});
  }
